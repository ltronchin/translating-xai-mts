...
for X_acc, X_pos, y, file_names in tqdm(data_loaders['valid']):
    ...
    with torch.no_grad():
        ...
        features_acc = encoder(X_acc.float())  # Prediction
        X_acc_rec = decoder(features_acc.float())
        X_acc_rec = torch.transpose(X_acc_rec, dim0=1, dim1=2).detach().cpu().numpy() # reshape to channel first

    X_acc_rec = tf.convert_to_tensor(X_acc_rec, dtype=tf.float32)
    X_pos = tf.convert_to_tensor(X_pos, dtype=tf.float32)

    pred = model.predict([X_acc_rec, X_pos])
